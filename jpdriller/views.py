...
def get_vocabulary(request):
    ...
    if groups == 'GETLAST':
        ...
        opt_list = []
        for i in range(0, Qnb):
            opt_list.append(vocab)
        ...
        opt = random.sample(range(0, len(vocab)-1), Qnb)
        
        opt_list = []
        for i in range(0, Qnb):
            opt_list.append(vocab[opt[i]])
        vocab = vocab[opt[random.randint(0, len(opt) - 1)]]
    ...
    response += ',' + str(vocab.id) + ','+ str(vocab.vocabulary) + ','+ str(vocab.pronunciation) + ','+ str(vocab.translation) + ','+ str(vocab.note) + ','+ str(vocab.group) + ','
    ...
    for i in range(0, Qnb):
        response += ','  + str(opt_list[i].vocabulary) + ','+ str(opt_list[i].pronunciation) + ','+ str(opt_list[i].translation)
    response += ','
